...
- hosts: {hosts_config}
  ...
  roles:
     ...
     - zimon/cluster
     - zimon/postcheck
- name: Include a play for OpenMPI
  import_playbook: openmpi.yml
- name: Include a play for Cluster Health Check
...
def prepare_ansible_playbook_openmpi(src_path):
    """ Write to playbook """
    content = """---
- name: Install and configure Open MPI
  hosts: scale_nodes
  tasks:
    - name: openmpi run configure
      ansible.builtin.shell:
        cmd: ./configure --prefix=/opt/ibm/openmpi --enable-script-wrapper-compilers --enable-wrapper-rpath --enable-orterun-prefix-by-default --with-slurm --enable-mpi-fortran=usempi
        chdir: {src_path}
    - name: openmpi run make
      ansible.builtin.shell:
        cmd: make -j 4
        chdir: {src_path}
    - name: openmpi run make install
      ansible.builtin.shell:
        cmd: make install
        chdir: {src_path}
    - name: Create symbolic link for openmpi
      ansible.builtin.shell:
        cmd: sudo ln -s /opt/ibm/openmpi/bin/* /usr/bin/
""".format(src_path=src_path)
    return content

def prepare_ansible_playbook_health_check(scale_mount_point, slurm_worker, management_node_ip):
    """ Write to playbook """
    content = """---
# Ensure scale storage is mounted and accessible and slurm node available
- name: Check scale storage
  hosts: scale_nodes
  tasks:
    - name: Check scale mount point available
      shell: mountpoint -q /gpfs/fs1
    - name: Create a file in scale mount point
      shell: touch /gpfs/fs1/{ansible_hostname}.txt
    - name: Check no of filed from management node
      shell: ls /gpfs/fs1/ | wc -l
      when: "'{management_node_ip}' in inventory_hostname"
      args:
        warn: false
      register: outresult
      failed_when: "'{scale_mount_point}' != outresult.stdout"
    - fail:
         msg: "Scale storage not accessible in all nodes"
      when:
         - "'{management_node_ip}' in inventory_hostname"
         - "'{scale_mount_point}' != outresult.stdout"
- name: Check slurm worker availability
  hosts: scale_nodes
  tasks:
    - name: Check Configured slurm workers
      when: "'{management_node_ip}' in inventory_hostname"
      shell: sinfo -N | awk '{{print $1}}' | grep "-" | wc -l
      args:
        warn: false
      register: result
      failed_when: "'{slurm_worker}' != result.stdout"
    - fail:
         msg: "Configured Slurm worker doesn't match the required worker count "
      when:
         - "'{management_node_ip}' in inventory_hostname"
         - "'{slurm_worker}' != result.stdout"
""".format(scale_mount_point=scale_mount_point,slurm_worker=slurm_worker,ansible_hostname='{{ansible_hostname}}', management_node_ip=management_node_ip)
    return content
...
def get_disks_list(az_count, disk_mapping, desc_disk_mapping,disk_type):
    ...
    if disk_type == "locally-attached":
        failureGroup = 0
        if len(TF['storage_cluster_instance_private_ips'][1:]) <= 20 :
            ...
            for each_ip, disk_per_ip in disk_mapping.items():
                ...
                if count_node == 10:
                    ...
                    half_Disk_Count = 1
                    for each_disk in disk_per_ip:
                        if half_Disk_Count <= 4:
                            disks_list.append({"device": each_disk,
                                   "failureGroup": failureGroup, "servers": each_ip,
                                   "usage": "metadataOnly", "pool": "system"})
                            half_Disk_Count = half_Disk_Count + 1
                        else:   
                            disks_list.append({"device": each_disk,
                                   "failureGroup": failureGroup, "servers": each_ip,
                                   "usage": "dataOnly", "pool": "data"})
                else:  
                    count = count + 1
                    for each_disk in disk_per_ip:
                        disks_list.append({"device": each_disk,
...
if __name__ == "__main__":
    ...
    if ARGUMENTS.using_packer_image == "false" and ARGUMENTS.using_rest_initialization == "true":
        ...
        openmpi_playbook_content = prepare_ansible_playbook_openmpi("/opt/src/openmpi_src")
        write_to_file("/%s/%s/openmpi.yml" % (ARGUMENTS.install_infra_path,
                                                        "ibm-spectrum-scale-install-infra"), openmpi_playbook_content)
        health_check_playbook_content = prepare_ansible_playbook_health_check(len(TF['storage_cluster_instance_private_ips']),len(TF['storage_cluster_instance_private_ips'][1:]),TF['storage_cluster_instance_private_ips'][0])
        write_to_file("/%s/%s/health_check.yml" % (ARGUMENTS.install_infra_path,
                                                        "ibm-spectrum-scale-install-infra"), health_check_playbook_content)
